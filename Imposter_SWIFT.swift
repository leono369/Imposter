...
struct RoundCard: Identifiable, Codable, Hashable {
    ...
    init(name: String, role: PlayerRole, display: String, sub: String, secret: String, masked: Bool) {
        self.id = UUID()
        self.name = name; self.role = role; self.display = display
        self.sub = sub; self.secret = secret; self.masked = masked
    }
}
...
let categories: [WordCategory] = [
    WordCategory(id: "alltag", name: "Alltag", words: [
    ...
    WordCategory(id: "tiere", name: "Tiere", words: [
    ...
    WordCategory(id: "essen", name: "Essen", words: [
    ...
    WordCategory(id: "berufe", name: "Berufe", words: [
    ...
    WordCategory(id: "getraenke", name: "Getränke", words: [
    ...
    WordCategory(id: "musik", name: "Musik", words: [
...
final class GameState: ObservableObject {
    ...
    let win = 14
    private let saveKey = "imposter_v2_swift"

    enum Screen { case menu, rules, setup, reveal, score }

    init() { loadSavedPreview() }
    ...
    func changePlayers(_ value: Int) {
        withAnimation(.spring(response: 0.35, dampingFraction: 0.7)) {
            ...
            while chosenCount() > maxSpecial() {
                for s in specials where roles[s.id] == true { roles[s.id] = false; break }
            }
    ...
    func startRoom() {
        ...
        buildRound(); revealIndex = 0; save()
        withAnimation(.spring(response: 0.45, dampingFraction: 0.75)) { screen = .reveal }
    }

    func nextRound() {
        roundNumber += 1
        winnerShown = false
        buildRound(); revealIndex = 0; rolesShown = false; save()
        withAnimation(.spring(response: 0.45, dampingFraction: 0.75)) { screen = .reveal }
    }

    func openScore() {
        rolesShown = false; save()
        withAnimation(.spring(response: 0.45, dampingFraction: 0.75)) { screen = .score }
        checkWinner()
    }
    ...
    func endRoom() {
        ...
        savedGame = nil; showWinnerOverlay = false; winnerShown = false
        withAnimation(.spring(response: 0.45, dampingFraction: 0.75)) { screen = .menu }
    }

    func save() {
        let s = GameSave(players: players, names: names, roles: roles, category: category,
                         scores: scores, roundNumber: roundNumber, assign: roundCards, theNum: theNum)
    ...
    func loadSavedPreview() {
        guard let data = UserDefaults.standard.data(forKey: saveKey),
              let s = try? JSONDecoder().decode(GameSave.self, from: data) else { savedGame = nil; return }
        savedGame = s
    }

    func resumeSession() {
        guard let s = savedGame else { return }
        players = s.players; names = s.names; roles = s.roles; category = s.category
        scores = s.scores; roundNumber = s.roundNumber; roundCards = s.assign; theNum = s.theNum
        rolesShown = false; winnerShown = false
        withAnimation(.spring(response: 0.45, dampingFraction: 0.75)) { screen = .score }
    }
    ...
    private func buildRound() {
        guard let cat = categories.first(where: { $0.id == category }) else { return }
        let otherCats = categories.filter { $0.id != category }
        theNum = Int.random(in: 1...20)
        let word = cat.words[theNum - 1]

        var pool: [PlayerRole] = [.imposter]
        for s in specials where roles[s.id] == true {
            if s.id == "saboteur"    { pool.append(.saboteur) }
            if s.id == "senior"      { pool.append(.senior) }
            if s.id == "doppelagent" { pool.append(.doppelagent) }
        }
        while pool.count < players { pool.append(.crew) }
        pool.shuffle()

        roundCards = pool.enumerated().map { index, role in
            var display = word
            var sub = "Kategorie · \(cat.name)"
            var secret = ""
            var masked = false

            switch role {
            case .imposter:
                display = "???"; sub = "Du kennst den Begriff nicht"
            case .saboteur:
                secret = "Dein Ziel: lass dich als Imposter verdächtigen!"
            case .senior:
                display = maskWord(word); masked = true; sub = "Lies genau hin …"
            case .doppelagent:
                if let other = otherCats.randomElement() {
                    display = other.words[theNum - 1]; sub = "Kategorie · \(other.name)"
                }
            case .crew: break
            }
            return RoundCard(name: playerName(index), role: role,
                             display: display, sub: sub, secret: secret, masked: masked)
        }
    }

    private func maskWord(_ word: String) -> String {
        var result = ""; var isStart = true
        for ch in word {
            if !ch.isLetter { result.append(ch); isStart = true }
            else if isStart { result.append(ch); isStart = false }
            else { result.append(Bool.random() ? "_" : ch) }
        }
        return result
    }
}
...
struct PremiumButtonStyle: ButtonStyle {
    ...
    private func bg(isPressed: Bool) -> some View {
        ...
        case .secondary:
            LinearGradient(colors: [.iosPurple, Color(red: 0.36, green: 0.24, blue: 0.59)],
...
struct FloatingBgBlob: View {
    let color: Color
    @State private var offset = CGSize(width: CGFloat.random(in: -50...50), height: CGFloat.random(in: -80...80))
        ...
        Circle().fill(color).frame(width: 200, height: 200)
            ...
            .onAppear {
                withAnimation(.easeInOut(duration: Double.random(in: 6...10)).repeatForever(autoreverses: true)) {
                    offset = CGSize(width: CGFloat.random(in: -80...80), height: CGFloat.random(in: -120...120))
    ...
    var body: some View {
        VStack(spacing: 0) {
            VStack(spacing: 12) {
                ...
                Text("Alle kennen den Begriff — nur einer nicht. Gib clevere Hinweise und entlarve, wer in Wahrheit nichts weiß.")
            ...
            HStack(spacing: 10) {
                FactView(icon: "text.quote",                value: "1",  label: "Begriff / Runde",
                         gradient: LinearGradient(colors: [.iosAccent, .iosPurple], startPoint: .top, endPoint: .bottom))
                FactView(icon: "person.2.badge.gearshape.fill", value: "4", label: "Sonderrollen",
                         gradient: LinearGradient(colors: [.iosPurple, Color(red:0.36,green:0.24,blue:0.59)], startPoint: .top, endPoint: .bottom))
                FactView(icon: "trophy.fill",               value: "14", label: "Punkte zum Sieg",
                         gradient: LinearGradient(colors: [.iosAccent, Color(red:0.6,green:0.7,blue:0)], startPoint: .top, endPoint: .bottom))
            }
            .padding(.top, 28)
            ...
            if let save = game.savedGame, save.roundNumber > 0 {
                ...
                .overlay(RoundedRectangle(cornerRadius: 20).stroke(Color.iosAccent.opacity(0.3), lineWidth: 1.5))
                .padding(.top, 24)
            }
            ...
            VStack(spacing: 12) {
                ...
                Button { withAnimation(.spring(response: 0.45, dampingFraction: 0.75)) { game.screen = .rules } }
                label: { Label("Spielregeln", systemImage: "book.closed.fill") }
...
struct FactView: View {
    let icon: String; let value: String; let label: String; let gradient: LinearGradient
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Block(title: "Worum geht's?") {
                Text("Alle bekommen denselben Begriff — nur der Imposter nicht. Mit cleveren Hinweisen müsst ihr ihn enttarnen, bevor er das Wort errät. Doch zwischen euch lauern noch fiese Sonderrollen …")
                    .font(.system(.body, design: .rounded)).foregroundColor(.white.opacity(0.8)).lineSpacing(4)
            }

            Block(title: "So läuft eine Runde") {
                VStack(alignment: .leading, spacing: 14) {
                    RuleStep(number: 1, text: "Karten verteilen — jeder sieht heimlich seine Rolle & seinen Begriff.")
                    RuleStep(number: 2, text: "Eine Zahl von 1–20 wird gezogen: das ist der Begriff der Runde.")
                    RuleStep(number: 3, text: "Reihum gibt jeder einen Hinweis zu seinem Begriff. Sobald alle einmal dran waren, ist eine Runde vorbei (2–3 Runden gesamt).")
                    RuleStep(number: 4, text: "Diskutieren & abstimmen: Wer ist der Imposter?")
                    RuleStep(number: 5, text: "Wenn alle Hinweis-Runden gespielt wurden: Die Person, die das Handy hält, öffnet die Auswertung und trägt die Punkte für alle ein. Sieg bei 14 Punkten.")
                }
            }

            Block(title: "So spielt ihr's digital") {
                Text("Ein Spielzimmer merkt sich eure Namen und Punkte rundenübergreifend. Ihr spielt am selben Handy (Pass & Play): Karten gehen reihum, jeder deckt seine eigene heimlich auf. Sobald alle Hinweis-Runden gespielt wurden, öffnet die Person, die das Handy gerade hält, mit dem Button „Fertig — zur Auswertung" die Punkteverteilung und tippt die Punkte für alle ein.")
                    .font(.system(.body, design: .rounded)).foregroundColor(.white.opacity(0.8)).lineSpacing(4)
            }

            Block(title: "Die Rollen") {
                VStack(alignment: .leading, spacing: 14) {
                    RoleRow(title: "Imposter",    desc: "Kennt den Begriff nicht. Muss improvisieren und ihn erraten, ohne aufzufliegen.", color: .iosAccent)
                    RoleRow(title: "Saboteur",    desc: "Kennt den Begriff — will aber absichtlich als Imposter verdächtigt werden.", color: .iosPurple)
                    RoleRow(title: "Senior",      desc: "Sieht den Begriff nur lückenhaft. Darf sich nichts anmerken lassen.", color: .iosPurple)
                    RoleRow(title: "Doppelagent", desc: "Bekommt einen komplett anderen Begriff — und weiß selbst nicht, wo er steht.", color: .iosPurple)
                    RoleRow(title: "Crew",        desc: "Kennt den echten Begriff und muss den Imposter enttarnen.", color: .iosGreen)
                }
            }
        }
    }
...
struct SetupView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack {
                Button { game.changePlayers(-1) } label: {
                ...
                Spacer()
                VStack(spacing: 2) {
                    Text("\(game.players)").font(.system(size: 48, weight: .black, design: .rounded)).foregroundColor(.white)
            ...
            Text(game.maxSpecial() == 0
                ...
                .font(.system(size: 13, weight: .medium, design: .rounded)).foregroundColor(.white.opacity(0.8))
                .padding(14).frame(maxWidth: .infinity, alignment: .leading)
                .background(Color.iosPurple.opacity(0.12)).cornerRadius(16)
                .overlay(RoundedRectangle(cornerRadius: 16).stroke(Color.iosPurple.opacity(0.35), lineWidth: 1.5))
                .padding(.top, 12)
                ...
                ForEach(0..<game.players, id: \.self) { i in
                    HStack(spacing: 12) {
                        ZStack {
                            ...
                            Text("\(i + 1)").font(.system(size: 13, weight: .bold, design: .rounded)).foregroundColor(.white)
                        }
                        TextField("Spieler \(i + 1)", text: Binding(
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 12) {
                    ForEach(categories) { cat in
                        ...
                        Button {
                            withAnimation(.spring(response: 0.35, dampingFraction: 0.7)) { game.category = cat.id }
                        } label: {
                            VStack(alignment: .leading, spacing: 12) {
                                HStack {
                                    ...
                                    Spacer()
                                    if sel { Image(systemName: "checkmark.circle.fill").foregroundColor(.white).font(.system(size: 18)) }
                                }
                                Spacer()
                                Text(cat.name).font(.system(.body, design: .rounded).bold()).foregroundColor(.white)
                                Text("\(cat.words.count) Begriffe").font(.system(.caption, design: .rounded)).foregroundColor(.white.opacity(0.8))
                            }
    ...
    private func categoryIcon(for id: String) -> String {
        switch id {
        case "alltag": return "house.fill"; case "tiere": return "pawprint.fill"
        case "essen": return "fork.knife"; case "berufe": return "briefcase.fill"
        case "getraenke": return "cup.and.saucer.fill"; case "musik": return "music.note"
    ...
    private func categoryGradient(for id: String) -> LinearGradient {
        switch id {
        case "alltag":    return LinearGradient(colors: [Color(red:0.2,green:0.8,blue:0.9), Color(red:0.4,green:0.3,blue:0.9)], startPoint: .topLeading, endPoint: .bottomTrailing)
        case "tiere":     return LinearGradient(colors: [.orange, .yellow], startPoint: .topLeading, endPoint: .bottomTrailing)
        case "essen":     return LinearGradient(colors: [.red, .orange], startPoint: .topLeading, endPoint: .bottomTrailing)
        case "berufe":    return LinearGradient(colors: [.blue, .teal], startPoint: .topLeading, endPoint: .bottomTrailing)
        case "getraenke": return LinearGradient(colors: [.iosPurple, .pink], startPoint: .topLeading, endPoint: .bottomTrailing)
        case "musik":     return LinearGradient(colors: [.iosPurple, Color(red:0.1,green:0.1,blue:0.6)], startPoint: .topLeading, endPoint: .bottomTrailing)
                ...
                VStack(spacing: 20) {
                    HStack(spacing: 6) {
                        ForEach(0..<game.roundCards.count, id: \.self) { i in
                            Capsule()
                                .fill(i == game.revealIndex ? Color.iosAccent
                    ...
                    Text("Gib das Handy an").font(.system(.headline, design: .rounded)).foregroundColor(.white.opacity(0.5))
                    ...
                    if cardOpen {
                        Button("Gesehen — weiter") {
                            withAnimation(.spring(response: 0.35, dampingFraction: 0.75)) {
                                cardOpen = false; game.revealIndex += 1
                            }
                        }
                        .buttonStyle(PremiumButtonStyle(variant: .primary))
...
struct FlippingCardView: View {
    let card: RoundCard; let number: Int
    @Binding var isFlipped: Bool

    var body: some View {
        ZStack {
            SecretCard(card: card, number: number).rotation3DEffect(.degrees(180), axis: (0,1,0)).opacity(isFlipped ? 1 : 0)
        ...
        .onTapGesture {
            withAnimation(.spring(response: 0.5, dampingFraction: 0.75, blendDuration: 0)) { isFlipped.toggle() }
        }
    }
}
...
struct CoverCard: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            Spacer()
            ZStack {
                Circle().stroke(LinearGradient(colors: [.iosPurple, .iosAccent], startPoint: .topLeading, endPoint: .bottomTrailing),
                                style: StrokeStyle(lineWidth: 2, dash: [8,4])).frame(width: 80, height: 80)
                Text(initial).font(.system(size: 32, weight: .black, design: .rounded))
                    .foregroundStyle(LinearGradient(colors: [.iosPurple, .iosAccent], startPoint: .topLeading, endPoint: .bottomTrailing))
            }
            VStack(spacing: 6) {
                Text("Deine Geheimkarte").font(.system(.headline, design: .rounded).bold()).foregroundColor(.white)
        ...
        .overlay(RoundedRectangle(cornerRadius: 28, style: .continuous)
            .stroke(LinearGradient(colors: [.iosPurple.opacity(0.4), .iosAccent.opacity(0.3)], startPoint: .topLeading, endPoint: .bottomTrailing), lineWidth: 2))
...
struct SecretCard: View {
    ...
    var body: some View {
        ZStack {
            VStack(spacing: 20) {
                ...
                VStack(spacing: 6) {
                    ...
                    Text(card.sub).font(.system(size: 15, weight: .medium, design: .rounded)).foregroundColor(.white.opacity(0.6))
                }
                if !card.secret.isEmpty {
                    Text(card.secret).font(.system(.subheadline, design: .rounded).bold())
                        .foregroundStyle(LinearGradient(colors: [.iosAccent, Color(red:0.6,green:0.7,blue:0)], startPoint: .leading, endPoint: .trailing))
            ...
            VStack {
                HStack(alignment: .center) {
                    HStack(spacing: 4) {
                        Image(systemName: "number").font(.system(size: 10, weight: .bold)).foregroundColor(.white.opacity(0.7))
                    ...
                    Text(card.role.label).font(.system(size: 12, weight: .black, design: .rounded))
                        .textCase(.uppercase).foregroundColor(card.role == .imposter ? Color(red:0.05,green:0.05,blue:0.04) : .white)
                        .padding(.horizontal, 12).padding(.vertical, 6)
        ...
        .frame(maxWidth: .infinity, maxHeight: .infinity)
        .background(RoundedRectangle(cornerRadius: 28, style: .continuous).fill(Color.cardBg))
        .overlay(RoundedRectangle(cornerRadius: 28, style: .continuous).stroke(roleColor(card.role).opacity(0.4), lineWidth: 2))
    ...
    var body: some View {
        VStack(spacing: 20) {
            ZStack {
                ...
                Image(systemName: "checkmark.seal.fill").font(.system(size: 40, weight: .bold)).foregroundColor(.iosGreen)
            }.padding(.bottom, 8)

            Text("Alle haben ihre Karte gesehen.").font(.system(.headline, design: .rounded)).foregroundColor(.white.opacity(0.6))
            ...
            Text("Jetzt reihum Hinweise geben (2–3 Runden). Danach diskutieren, abstimmen — und dann die Auswertung öffnen.")
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if let idx = winnerIndex {
                ...
                .background(LinearGradient(colors: [Color.iosAccent.opacity(0.9), Color(red:0.6,green:0.7,blue:0)], startPoint: .top, endPoint: .bottom))
                .cornerRadius(20)
            ...
            VStack(spacing: 12) {
                Button("Nächste Runde") { game.nextRound() }.buttonStyle(PremiumButtonStyle(variant: .primary))
                Button("Spielzimmer beenden") { showEndAlert = true }.buttonStyle(PremiumButtonStyle(variant: .outline))
            }
            .padding(.top, 28)
        }
        ...
        } message: { Text("Punkte gehen verloren.") }
    }
}
        ...
        VStack(spacing: 12) {
            HStack(spacing: 12) {
                VStack(alignment: .leading, spacing: 4) {
                    HStack(spacing: 8) {
                        ...
                        if isLead { Image(systemName: "crown.fill").foregroundColor(.iosAccent).font(.system(size: 14)) }
                    }
                ...
                HStack(spacing: 6) {
                    Button {
                        withAnimation(.spring(response: 0.3, dampingFraction: 0.6)) { game.adjust(index, by: -1) }
                    } label: {
                    ...
                    .overlay(RoundedRectangle(cornerRadius: 8).stroke(isLead ? Color.iosAccent.opacity(0.5) : .clear, lineWidth: 1))

                    Button {
                        withAnimation(.spring(response: 0.3, dampingFraction: 0.6)) { game.adjust(index, by: 1) }
                    } label: {
                        Text("+1").font(.system(size: 13, weight: .black, design: .rounded)).foregroundColor(Color(red:0.05,green:0.05,blue:0.04))
                            .frame(width: 36, height: 32).background(Color.iosAccent).cornerRadius(8)
                    }.buttonStyle(.plain)

                    Button {
                        withAnimation(.spring(response: 0.3, dampingFraction: 0.6)) { game.adjust(index, by: 3) }
                    } label: {
                        Text("+3").font(.system(size: 13, weight: .black, design: .rounded)).foregroundColor(.white)
                            .frame(width: 36, height: 32)
                            .background(LinearGradient(colors: [.iosPurple, Color(red:0.36,green:0.24,blue:0.59)], startPoint: .top, endPoint: .bottom))
                            .cornerRadius(8)
                    }.buttonStyle(.plain)
                }
            }
            ...
            GeometryReader { geo in
                ZStack(alignment: .leading) {
                    ...
                    Capsule()
                        .fill(LinearGradient(
                            colors: isLead ? [.iosAccent, Color(red:0.6,green:0.7,blue:0)] : [.iosPurple, Color(red:0.36,green:0.24,blue:0.59)],
                            startPoint: .leading, endPoint: .trailing))
                        .frame(width: geo.size.width * CGFloat(min(1.0, Double(score) / Double(game.win))), height: 6)
                }
            }.frame(height: 6)
        }
        .padding(14).background(Color.cardBg).cornerRadius(18)
        .overlay(RoundedRectangle(cornerRadius: 18).stroke(isLead ? Color.iosAccent.opacity(0.4) : Color.borderLine, lineWidth: 1.5))
        .shadow(color: isLead ? Color.iosAccent.opacity(0.08) : .clear, radius: 10, y: 5)
    }

    private func roleText(_ card: RoundCard) -> String {
        card.role == .imposter || card.role == .crew ? card.role.label : "\(card.role.label) · \(card.display)"
    }
}
    ...
    var body: some View {
        Group {
            if data.isCircle {
                ...
                RoundedRectangle(cornerRadius: 1.5).fill(data.color).frame(width: data.width, height: data.height)
            }
        }
...
struct WinnerOverlayView: View {
    ...
    var body: some View {
        ZStack {
            ...
            .cornerRadius(28)
            .overlay(RoundedRectangle(cornerRadius: 28).stroke(Color.iosAccent.opacity(0.45), lineWidth: 1.5))
        ...
        .onAppear {
            ...
            withAnimation(.spring(response: 0.6, dampingFraction: 0.58).delay(0.4)) {
                trophyOffset = 0; trophyOpacity = 1.0
            }
        }
    }
}
